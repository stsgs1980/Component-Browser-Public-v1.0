...
- **290 компонентов**, **20 категорий**, **~71 000 строк** React/TypeScript кода (287 файлов на диске)
...
- Компоненты с broken imports (точное число — в браузере / POST /api/scan)
...
- Компоненты без Props interface (точное число — в браузере / POST /api/scan)
...
- Компоненты без JSDoc (точное число — в браузере / POST /api/scan)
...
- Файлов > 500 строк (точное число — в браузере / POST /api/scan)
...
| Clean статус | динамически* | 70%+ | 85%+ | 90%+ |
| Broken imports | динамически* | **0** | 0 | 0 |
| С Props interface | динамически* | 80%+ | 95%+ | 95%+ |
| С JSDoc | динамически* | 70%+ | 90%+ | 95%+ |
| Live Preview | реализовано | 30%+ | 60%+ | 70%+ |
| Файлов > 500 строк | динамически* | **0** | 0 | 0 |
| Collections | 0 | 3 | 6+ | 10+ |
| CLI команды | 0 | `install` | `install`, `add` | `install`, `add`, `update`, `list` |

> *Метрики с пометкой «динамически» вычисляются сканером при каждом запуске. Точные значения см. в браузере (sidebar badges) или через `POST /api/scan`.
